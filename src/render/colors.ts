 ...
 */
function sampleGradientRgb(palette: string[], position: number): { r: number; g: number; b: number } {
  ...
  if (lo === hi) return hexToRgb(palette[lo]);
  const t = clamped - lo;
...
function ghostColorAt(position: number, width: number, palette: string[]): string {
  if (palette.length === 0) return '';
  const hex = palette.length === 1 || width <= 1
    ? palette[0]
    : palette[Math.min(
        palette.length - 1,
        Math.max(0, Math.round((position / (width - 1)) * (palette.length - 1))),
      )];
